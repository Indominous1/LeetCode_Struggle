...
A count-paired domain is a domain that has one of the two formats "rep d1.d2.d3" or "rep d1.d2" where rep is the number of visits to the domain and d1.d2.d3 is the domain 
itself.
...
public:
    vector<string> subdomainVisits(vector<string>& d) {
        ...
        {
            string s=d[i];
            int j=s.find(' '),k=stoi(s.substr(0,j));
            string o=s.substr(j+1);
            mp[o]+=k;
